...
public class UserRestController {
    ...
    public ResponseEntity< Map<String,Object> > login(@RequestBody UserDto userDto, HttpServletResponse resp) throws IOException {
        ...
        try{
            ...
            //REDIS 에 REFRESH 저장
            redisUtil.save(authentication.getName() , tokenInfo.getRefreshToken());
            //
            response.put("state","success");
            response.put("message","인증성공!");

            //---------------------------------------------
            Cookie accessCookie = new Cookie(JwtProperties.ACCESS_TOKEN_NAME, tokenInfo.getAccessToken());
            ...
            Cookie refreshCookie = new Cookie(JwtProperties.REFRESH_TOKEN_NAME, tokenInfo.getRefreshToken());
            refreshCookie.setHttpOnly(true);
            accessCookie.setSecure(false); // Only for HTTPS
            refreshCookie.setPath("/");
            refreshCookie.setMaxAge(JwtProperties.EXPIRATION_TIME_REFRESH); // 7 days expiration

            Cookie userCookie = new Cookie("username", authentication.getName());
            userCookie.setHttpOnly(true);
            accessCookie.setSecure(false); // Only for HTTPS
            userCookie.setPath("/");
            userCookie.setMaxAge(JwtProperties.EXPIRATION_TIME_REFRESH); // 7 days expiration
